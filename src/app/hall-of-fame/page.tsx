                  ...
                  <div className="bg-black/40 rounded-[22px] p-6 md:p-10">
                    ...
                    {(season as any).sponsors && (
                      <div className="mt-8 pt-6 border-t border-white/5">
                        <div className="text-sm font-bold text-[#b0b8d4] uppercase tracking-widest mb-3 flex items-center gap-2">
                          <span className="text-[#d4af37]">★</span> Official Sponsors
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {(season as any).sponsors.split(',').map((sponsor: string, i: number) => (
                            <span key={i} className="px-4 py-1.5 bg-gradient-to-r from-white/10 to-transparent border border-white/10 rounded-full text-xs text-white font-semibold tracking-wide">
                              {sponsor.trim()}
                            </span>
                          ))}
                        </div>
